"""Craigslist scraper using BeautifulSoup with requests.

Searches all metro areas within 200 miles of zip 41051.
"""
...
from .base import (
    SCRAPING, VEHICLE, random_delay, random_user_agent, parse_mileage,
    parse_price, parse_year,
)
...
def scrape_sync() -> list[dict]:
    """Synchronous scrape of all Craigslist metros. Called from async wrapper."""
    all_listings = []
    ...
    for metro in METROS:
        ...
        try:
            listings = _scrape_metro(url, name)
...
def _scrape_metro(url: str, metro_name: str) -> list[dict]:
    listings = []
    max_retries = SCRAPING["max_retries"]
    backoff = SCRAPING["backoff_seconds"]
    ...
    for attempt in range(max_retries):
        try:
            ...
            if resp.status_code == 429 or resp.status_code == 503:
                ...
                logger.warning(
                    f"[{SOURCE_NAME}] {metro_name} rate limited (HTTP {resp.status_code}). "
                    f"Waiting {wait}s..."
                )
                time.sleep(wait)
                continue
            ...
            if attempt < max_retries - 1:
                wait = backoff[min(attempt, len(backoff) - 1)]
                logger.warning(f"[{SOURCE_NAME}] {metro_name} request failed: {e}. Retrying in {wait}s...")
                time.sleep(wait)
            else:
                logger.error(f"[{SOURCE_NAME}] {metro_name} all retries exhausted: {e}")
                return []
    else:
        return []
    ...
    # Try new Craigslist layout first
    results = soup.select("li.cl-static-search-result, li.cl-search-result, .result-row")
...
def _parse_result(result, metro_name: str) -> dict | None:
    ...
    title_el = result.select_one(
        ".title, .titlestring, a.titlestring, .result-title, a.posting-title"
    )
    if title_el:
    ...
    # Mileage — often in subtitle or metadata
    ...
    # Location
    loc_el = result.select_one(".meta .location, .result-hood, .nearby")
    if loc_el:
